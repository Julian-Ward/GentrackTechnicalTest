...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 12 23:28:26 2022

These functions convert a local XML file to a CSV file following the 
...
def xmlRead(filename):
    #   Interprets XML CSVDataIntervals -> string
    #   XML: Transactions->Transaction->MeterDataNotification->CSVIntervalData
...
def mapDataNodes(cleanlist):
    #   Finds and maps 100,200,300,900 nodes in CSVdata
...
def makeHeader(cleanlist):
    #   Makes header row for CSV file
    
    headerRow = cleanlist[find100[0]:find200[0]]
    return headerRow

def makeTrailer(cleanlist):
    #   Makes the trailer row for CSV file
    
    trailerRow = cleanlist[find900[0]]
    return trailerRow

def writeCSVFile(headerRow, trailerRow, cleanlist):
        #   Works through cleaned XML file to make the CSV file(s)
...
def convertXMLtoCSV(filename):
    #   converts input Gentrack XML file to Gentrack CSV file format
    
    csvdata = xmlRead(filename)
    cleanlist = dataCleanUp(csvdata)
    mapDataNodes(cleanlist)
    header = makeHeader(cleanlist)
    trailer = makeTrailer(cleanlist)
    writeCSVFile(header, trailer, cleanlist)
